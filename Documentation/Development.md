...
9. Then perform the following command in your text editor terminal to install reportgenerator (for backend testing)

    `dotnet tool install dotnet-reportgenerator-globaltool`
   
11. Your local file now be set up.
...
### test
This is where the backend testing lives and where a script lives for total testing.
...
# Testing
You can find tests for Backend in src/test and tests for Frontend in src/spa-template-vue/src/components/\_test\_. You can find documentation for Backend in src/test/README.md and for Frontend in src/spa-template-vue/TESTING.md

You can then follow the instructions located in those documents. Here is what they say semi-verbatim:
...
```powershell
npm run test:unit -- rothOptimizationService.spec
```

### Run tests matching a pattern

```powershell
npm run test:unit -- --grep "optimization"
```
...
Open in browser to see detailed line-by-line coverage for all files.

## Backend
To run dotnet tests (for coverage) perform the following lines:
```powershell
cd src/
dotnet test --collect:"XPlat Code Coverage"
reportgenerator -reports:**/coverage.cobertura.xml -targetdir:test/coveragereport -reporttypes:Html
start test/coveragereport/index.html
```

To auto run testing (not coverage):
```powershell
dotnet watch test
```
...
You can perform the following one-liner to use a script (pending release to testing branch) that runs all tests and opens all reports generated by them
```powershell
.\src\test\test_script.ps1
```
NOTE: If the Vue tests fail they will not produce an HTML file to read.
...
5. Then perform the following commands in your text editor terminal to install yarn
   ...
   `Invoke-WebRequest https://repo.yarnpkg.com/4.2.2/packages/yarnpkg-cli/bin/yarn.js -OutFile .yarn\releases\yarn-4.2.2.cjs`
6. Your local file now be set up.
...
5. Select the optimization 'Maximize Capital', enter your parameters, and hit 'Next'
6. You should see a text box containing a strategy, warnings, a graph, tables, highlighted numbers etc.
7. Go back to the home page and click on 'Client Creation', this should bring you to a new page where you can enter information. If you can create a client, then everything is working.
